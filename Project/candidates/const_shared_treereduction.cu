...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    #define tree_reduce_3d(i2,i1,i0) shared_mem[ (i2) * (BLOCK_SIZE*Channel) + (i1)*Channel + i0]
    ...
    if (h_out < Height_out && w_out < Width_out){
        ...
        for (int ky = 0; ky < K; ky++){
            for (int kx = 0; kx < K; kx++){
                result += in_4d(batch_out, channel, h_out + ky, w_out + kx) * mask_4d(feature_out, channel, ky, kx);
            }
        }
        tree_reduce_3d(ty, tx, channel) = result;
        // based on the summation is what we want, we can use tree reduction to sum up so that sum can be done in logarithm time
        // for (unsigned int stride = Channel/2; stride >=1; stride >>= 1){
        for (unsigned int stride=1; stride<Channel ; stride<<=1){
            __syncthreads();
            if ((tz % (2 * stride) == 0) && tz+stride<Channel){ 
                tree_reduce_3d(ty, tx, tz) += tree_reduce_3d(ty, tx, tz + stride);
            }
        }
        __syncthreads();
        if (tz == 0){ // wait till all is done, and let the first channel do the 
            out_4d(batch_out, feature_out, h_out, w_out) = tree_reduce_3d(ty, tx, 0);
        }
    }
    #undef out_4d
    #undef in_4d
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE, Channel);
    conv_forward_kernel<<<dimGrid, dimBlock, Channel*BLOCK_SIZE*BLOCK_SIZE*sizeof(float)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
}
