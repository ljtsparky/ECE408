...
=======
  ...
  if (row < dim){
    float dot = 0;
    unsigned int sec = 0;//section number initially 0
    while (sec < matRows[row]){// section number means how many element in a specific row have, only go into the while loop when this row is in need this section
      dot += matData[matColStart[sec] + row] * vec[matCols[matColStart[sec] + row]];//as long as row is allowed to enter while, there must be a matcolstart[sec]+row for it
      sec++;
    }
...
=======
  ...
  //each thread one role, therefore 
  dim3 dimGrid(ceil((float)dim/BLOCK_WIDTH),1,1);//dim is the number of rows, one thread for each row
...
=======
           &hostJDSColStart, &hostJDSCols, &hostJDSData); //this is done?
>>>>>>> Stashed changes
...
=======
    //it seems that nothing needed here...
>>>>>>> Stashed changes
